...
class PDF :
    ...
    def __init__(self, df_cisco,df_cisco_in_dren, df_dren, d_mada):
        self.df_cisco = df_cisco
        self.df_cisco_in_dren = df_cisco_in_dren
    ...
    def y_zap_cisco(self):
        y = []
        for _,row in self.df_cisco_in_dren.iterrows() :
            data_y = {}
            data_y["CISCO"] = row.get("CISCO","")
            data_y["CODE_CISCO"] = row.get("CODE_CISCO","")
            try:
                ...
                data_y["Y"] = 50
                print(f'erreur pour {data_y["CODE_CISCO"]} :=> {e}')
                
            y.append(data_y)
        return y
    
    def x_zap_cisco(self):
        x = []
        for _,row in self.df_cisco_in_dren.iterrows() :
            try:
                data_x = {}
                data_x["CISCO"] = row.get("CISCO","")
                data_x["CODE_CISCO"] = row.get("CODE_CISCO","")
                ...
                data_x["X"] = 50
                print(f'erreur x_zaps pour {data_x["CODE_CISCO"]}: => {e}')
            x.append(data_x)
    ...
    def diagnostic_efficience(self, x, y):
        ...
        cas = {
            ("minim", "minim"): "<b>Faibles ressources et faibles résultats : </b>la CISCO  est en grande difficulté. Elle manque de moyens et cela se reflète directement dans les performances.",
            ("minim", "moyen"): "<b>Ressources faibles mais résultats moyens :</b> la CISCO  démontre une bonne efficience, elle réussit à faire progresser les élèves malgré des moyens limités.",
            ("minim", "maxim"): "<b>Ressources faibles mais résultats élevés :</b> situation exceptionnelle d’excellente efficience. la CISCO  exploite remarquablement bien ses ressources limitées.",
            ...
            ("moyen", "moyen"): "<b>Ressources et résultats moyens </b>: situation intermédiaire, la CISCO  fonctionne normalement mais sans véritable point fort ni faiblesse majeure.",
            ...
            ("maxim", "maxim"): "<b>Ressources et résultats élevés :</b> situation idéale. la CISCO  a les moyens et les utilise efficacement pour obtenir de très bons résultats."
        }
        ...
        if (niveau_x, niveau_y) == ("maxim", "maxim"):
            diagnostic = (
                "la CISCO dispose de ressources abondantes et parvient à obtenir d'excellents résultats. "
            ...
            diagnostic = (
                "Malgré des ressources très limitées, la CISCO atteint des résultats remarquables. "
            ...
            diagnostic = (
                "la CISCO réussit à obtenir des résultats corrects avec peu de moyens. "
            ...
            diagnostic = (
                ...
                "la CISCO démontre une bonne organisation et une pédagogie efficace. "
                "Il s'agit d'une situation très satisfaisante."
            )
        elif (niveau_x, niveau_y) == ("moyen", "moyen"):
            diagnostic = (
                "la CISCO fonctionne normalement avec des ressources et des résultats dans la moyenne. "
            ...
            if (niveau_x, niveau_y) == ("maxim", "moyen"):
                diagnostic = (
                    "la CISCO dispose de ressources importantes mais n'obtient que des résultats moyens. "
                ...
                diagnostic = (
                    "la CISCO dispose de moyens corrects mais n'arrive pas à transformer ces ressources en résultats. "
                ...
                diagnostic = (
                    "la CISCO est en grande difficulté, avec peu de ressources et de très faibles résultats. "
    ...
    def interpretation_abandon(self,ecole: float, zap: float) -> str:
        ...
        et le compare à celui de la CISCO (commune).
        ...
        if ecole < zap * 0.9:  # nettement meilleur (au moins 10% de différence relative)
            comparaison = "la CISCO fait nettement mieux que la moyenne de la commune (ZAP)."
            ...
            comparaison = "la CISCO est globalement au même niveau que la commune."
            suggestion = "Renforcer les actions locales conjointes avec la CISCO pour améliorer encore la rétention."
        else:  # moins bon
            comparaison = "la CISCO est en moins bonne situation que la commune."
    ...
    def export_pdf(self):
        df = self.df_cisco.copy()
        # Nettoyer les quotes et apostrophes dans CISCO
        ...
        row_mada = self.d_mada.iloc[0]

        df["CISCO"] = df["CISCO"].str.replace('"', '', regex=False).str.replace("'", '', regex=False)
        pdf_name = df["CISCO"].iloc[0].replace("'", '').replace('"', '').replace("'", '').replace("/", '-')
        cname = df.iloc[0]["CISCO"].replace('"', '').replace("'", '').replace("/", '-')
        ...
        CISCO_DIR = os.path.join(DREN_DIR, f'{cname}')
        os.makedirs(CISCO_DIR, exist_ok=True)
        CODE_CISCO =  df.iloc[0]['CODE_CISCO']
        output_path = os.path.join(CISCO_DIR, f'TDB_CISCO_{pdf_name}.pdf')
        ...
        zap = Paragraph(f'', normal_style)
        code = Paragraph(f'<b>CODE : {df.iloc[0]["CODE_CISCO"]}</b>', centered_style)
        ...
        data_abandon = [
            ...
            entetes,
            ['', '', '', ''],
            ['CP1->CP2', f'{row_cisco.get("txAbdcp1cp2","-1")}% ', f'{row_dren.get("txAbdcp1cp2","-1")}% ', f'{row_mada.get("txAbdcp1cp2","-1")}% '],
            ['CP2->CE', f'{row_cisco.get("txAbdcp2ce","-1")}% ', f'{row_dren.get("txAbdcp2ce","-1")}% ', f'{row_mada.get("txAbdcp2ce","-1")}% '],
            ['CE->CM1', f'{row_cisco.get("txAbdcecm1","-1")}% ', f'{row_dren.get("txAbdcecm1","-1")}% ', f'{row_mada.get("txAbdcecm1","-1")}% '],
            ['CM1->CM2', f'{row_cisco.get("txAbdcm1cm2","-1")}% ', f'{row_dren.get("txAbdcm1cm2","-1")}% ', f'{row_mada.get("txAbdcm1cm2","-1")}% '],
            ['Ensemble', f'{row_cisco.get("txAbdGlobal","-1")}% ', f'{row_dren.get("txAbdGlobal","-1")}% ', f'{row_mada.get("txAbdGlobal","-1")}% '],
        ]
        #couleur pour txAbdon ensemble :
        e,z = float(str(row_cisco.get("txAbdGlobal","-1")).replace(",",".")),float(str(row_dren.get("txAbdGlobal","-1")).replace(",","."))
        ...
        table_abandon.setStyle(TableStyle([
            ...
            ("TOPPADDING", (0, 0), (-1, -1), 2),
            ("GRID", (0, 0), (-1, -1), 1, colors.black),
            ("BACKGROUND", (1, -1), (1, -1), bc), #comparaison Ensemble Cisco Vs DREN
        ]))

        # Tableaux pourcentage redoublants **********************************************************
        redcp1 = row_cisco.get("red_CP1_g","-1") 
        redcp2 = row_cisco.get("red_CP2_g","-1") 
        redce = row_cisco.get("red_CE_g","-1")  
        redcm1 = row_cisco.get("red_CM1_g","-1")  
        redcm2 = row_cisco.get("red_CM2_g","-1") 
        
        data_pred = [
            [Paragraph('Pourcentage des redoublants', entete_style), '', '', ''],
            entetes,
            ['', '', '', ''],
            ['CP1', f'{redcp1}%', row_dren.get('red_CP1_g','-1'), row_mada.get('red_CP1_g','-1')],
            ['CP2', f'{redcp2}%', row_dren.get('red_CP2_g','-1'), row_mada.get('red_CP2_g','-1')],
            ['CE', f'{redce}%', row_dren.get('red_CE_g','-1'), row_mada.get('red_CE_g','-1')],
            ['CM1', f'{redcm1}%', row_dren.get('red_CM1_g','-1'), row_mada.get('red_CM1_g','-1')],
            ['CM2', f'{redcm2}%', row_dren.get('red_CM2_g','-1'), row_mada.get('red_CM2_g','-1')],
        ]
        ...
        rg_e = float(str(row_cisco.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_e = float(str(row_cisco.get("txRetentionFilles","0")).strip().replace(",","."))
        r_e = float(str(row_cisco.get("txRetentionTotal","0")).strip().replace(",","."))
        rg_z = float(str(row_dren.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_z = float(str(row_dren.get("txRetentionFilles","0")).strip().replace(",","."))
        r_z = float(str(row_dren.get("txRetentionTotal","0")).strip().replace(",","."))
        rg_c = float(str(row_mada.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_c = float(str(row_mada.get("txRetentionFilles","0")).strip().replace(",","."))
        r_c = float(str(row_mada.get("txRetentionTotal","0")).strip().replace(",","."))
        ...
        table_ret.setStyle(TableStyle([
            ...
            ("TOPPADDING", (0, 0), (-1, -1), 2),
            ("GRID", (0, 0), (-1, -1), 1, colors.black),
            ("BACKGROUND", (1, 3), (1, 3), bc), #comparaison Ensemble Cisco Vs DREN
        ]))

        # Tableaux pourcentage des redoublants par genre*********************************
        rg_e = row_cisco.get("red_garcons",-1) 
        rf_e = row_cisco.get("red_fille",-1) 
        r_e = row_cisco.get("red_ensemble",-1) 

        rg_z = row_dren.get("red_garcons",-1) 
        rf_z = row_dren.get("red_fille",-1) 
        r_z = row_dren.get("red_ensemble",-1) 


        rg_c = row_mada.get("red_garcons",-1) 
        rf_c = row_mada.get("red_fille",-1) 
        r_c = row_mada.get("red_ensemble",-1) 
        ...
        entetes = [
            ["Matières", "Cisco", "", "Dren", "", "Mada", ""],
            ["", "SM", "Note >= 10", "SM", "Note >= 10", "SM", "Note >= 10"]
        ]
        tx_admis_g =  row_cisco.get("tx_admis_g", 0)
        tx_admis_f = row_cisco.get("tx_admis_f", 0)
        tx_admis = row_cisco.get("tx_admis", 0)

        tx_admis_g_z =  row_dren.get("tx_admis_g", 0)
        tx_admis_f_z = row_dren.get("tx_admis_f", 0)
        tx_admis_z = row_dren.get("tx_admis", 0)

        tx_admis_g_c =  row_mada.get("tx_admis_g", 0)
        tx_admis_f_c = row_mada.get("tx_admis_f", 0)
        tx_admis_c = row_mada.get("tx_admis", 0)
        ...
        data_exam = [
            [titre_ex, "", "", "", "", "", ""], #0 
            ["Matières", "Cisco", "", "Dren", "", "Mada", ""], #1
            ["", "SM", " >= 10", "SM", " >= 10", "SM", " >= 10"], #2
            ["Malagasy", row_cisco.get("sm_mlg",0), row_cisco.get("sup_10_mlg",0), row_dren.get("sm_mlg",0), row_dren.get("sup_10_mlg",0),row_mada.get("sm_mlg",0), row_mada.get("sup_10_mlg",0)], #3
            ["  -Fahazoana lahatsoratra", "", "", "", "", "", ""], #4
            ["  -Fitsipika", "", "", "", "", "", ""], #5
            ["  -Fanazarana hanoratra", "", "", "", "", "", ""], #6
            ["Français", row_cisco.get("sm_fr",0), row_cisco.get("sup_10_fr",0), row_dren.get("sm_fr",0), row_dren.get("sup_10_fr",0),row_mada.get("sm_fr",0), row_mada.get("sup_10_fr",0)], #7
            ["Mathématiques", row_cisco.get("sm_maths",0), row_cisco.get("sup_10_maths",0), row_dren.get("sm_maths",0), row_dren.get("sup_10_maths",0),row_mada.get("sm_maths",0), row_mada.get("sup_10_maths",0)], #8
            ["  -Opération", row_cisco.get("sm_op",0), row_cisco.get("sup_10_op",0), row_dren.get("sm_op",0), row_dren.get("sup_10_op",0),row_mada.get("sm_op",0), row_mada.get("sup_10_op",0)], #9
            ["  -Problème", row_cisco.get("sm_probleme",0), row_cisco.get("sup_10_probleme",0), row_dren.get("sm_probleme",0), row_dren.get("sup_10_probleme",0),row_mada.get("sm_probleme",0), row_mada.get("sup_10_probleme",0)], #10
            ["Histroire", row_cisco.get("sm_tfm",0), row_cisco.get("sup_10_tfm",0), row_dren.get("sm_tfm",0), row_dren.get("sup_10_tfm",0),row_mada.get("sm_tfm",0), row_mada.get("sup_10_tfm",0)], #11
            ["Geographie", row_cisco.get("sm_geo",0), row_cisco.get("sup_10_geo",0), row_dren.get("sm_geo",0), row_dren.get("sup_10_geo",0),row_mada.get("sm_geo",0), row_mada.get("sup_10_geo",0)], #12
            ["SVT", row_cisco.get("sm_svt",0), row_cisco.get("sup_10_svt",0), row_dren.get("sm_svt",0), row_dren.get("sup_10_svt",0),row_mada.get("sm_svt",0), row_mada.get("sup_10_svt",0)], #13
            [ttl2, "", "", "", "", "", ""], #14
        ...
        bc_mlg = colors.red if row_cisco.get("sm_mlg",0) < 10 else colors.white
        bc_fr = colors.red if row_cisco.get("sm_fr",0) < 10 else colors.white
        bc_maths = colors.red if row_cisco.get("sm_maths",0) < 10 else colors.white
        bc_op = colors.red if row_cisco.get("sm_op",0) < 10 else colors.white
        bc_pb = colors.red if row_cisco.get("sm_probleme",0) < 10 else colors.white
        bc_tfm = colors.red if row_cisco.get("sm_tfm",0) < 10 else colors.white
        bc_geo = colors.red if row_cisco.get("sm_geo",0) < 10 else colors.white
        bc_svt = colors.red if row_cisco.get("sm_svt",0) < 10 else colors.white
        ...
        nbr_eleves = row_cisco.get("nombre_eleves", 0)
        ens_classe = row_cisco.get("ens_classe", 0)
        ens_im = row_cisco.get("ens_im", 0)
        fram_sub = row_cisco.get("fram_sub", 0)
        fram_nonsub = row_cisco.get("fram_nonsub", 0)
        nombre_section = row_cisco.get("nombre_section", 0)
        ecole_continue = row_cisco.get("ecole_continue", 0)
        eleve_2km = row_cisco.get("eleve_2km", 0)
        point_eau = row_cisco.get("point_eau", 0)
        electricite = row_cisco.get("electricite", 0)

        nbr_eleves_z = row_dren.get("nombre_eleves", 10)
        ens_classe_z = row_dren.get("ens_classe", 10)
        ens_im_z = row_dren.get("ens_im", 10)
        fram_sub_z = row_dren.get("fram_sub", 10)
        fram_nonsub_z = row_dren.get("fram_nonsub",10)
        nombre_section_z = row_dren.get("nombre_section", 10)
        ecole_continue_z = row_dren.get("ecole_continue", 10)
        eleve_2km_z = row_dren.get("eleve_2km", 0)
        point_eau_z = row_dren.get("point_eau", 0)
        electricite_z = row_dren.get("electricite", 0)

        nbr_eleves_c = row_mada.get("nombre_eleves", 10)
        ens_classe_c = row_mada.get("ens_classe", 10)
        ens_im_c = row_mada.get("ens_im", 10)
        fram_sub_c = row_mada.get("fram_sub", 10)
        fram_nonsub_c = row_mada.get("fram_nonsub",10)
        nombre_section_c = row_mada.get("nombre_section", 10)
        ecole_continue_c = row_mada.get("ecole_continue", 10)
        eleve_2km_c = row_mada.get("eleve_2km", 0)
        point_eau_c = row_mada.get("point_eau", 0)
        electricite_c = row_mada.get("electricite", 0)
        ...
        data_g = [
            ...
            ["Contexte","","",""],
            ["Ecole continue",f"{ecole_continue}%",f"{ecole_continue_z}%",f"{ecole_continue_c}%"],
            ["Elèves vivant à moins de 2km de leur CISCO",f"{eleve_2km}%",f"{eleve_2km_z}",f"{eleve_2km_c}%"],
        ...
        ratio_em = row_cisco.get("ratio_em", 0)
        ratio_eq = row_cisco.get("ratio_eq", 0)
        ratio_cpsdc = row_cisco.get("ratio_cpsdc", 0)
        ratio_epa = row_cisco.get("ratio_epa", 0)
        ratio_wc_com = row_cisco.get("ratio_wc_com", 0)
        ratio_wc_f = row_cisco.get("ratio_wc_f", 0)
        ratio_emlg = row_cisco.get("ratio_emlg", 0)
        ratio_emths = row_cisco.get("ratio_emths", 0)
        ratio_efrs = row_cisco.get("ratio_efrs", 0)
        montant_ce = row_cisco.get("montant_ce", 0)

        ratio_em_z = row_dren.get("ratio_em", 0)
        ratio_eq_z = row_dren.get("ratio_eq", 0)
        ratio_cpsdc_z = row_dren.get("ratio_cpsdc", 0)
        ratio_epa_z = row_dren.get("ratio_epa", 0)
        ratio_wc_com_z = row_dren.get("ratio_wc_com", 0)
        ratio_wc_f_z = row_dren.get("ratio_wc_f", 0)
        ratio_emlg_z = row_dren.get("ratio_emlg", 0)
        ratio_emths_z = row_dren.get("ratio_emths", 0)
        ratio_efrs_z = row_dren.get("ratio_efrs", 0)
        montant_ce_z = row_dren.get("montant_ce", 0)

        ratio_em_c = row_mada.get("ratio_em", 0)
        ratio_eq_c = row_mada.get("ratio_eq", 0)
        ratio_cpsdc_c = row_mada.get("ratio_cpsdc", 0)
        ratio_epa_c = row_mada.get("ratio_epa", 0)
        ratio_wc_com_c = row_mada.get("ratio_wc_com", 0)
        ratio_wc_f_c = row_mada.get("ratio_wc_f", 0)
        ratio_emlg_c = row_mada.get("ratio_emlg", 0)
        ratio_emths_c = row_mada.get("ratio_emths", 0)
        ratio_efrs_c = row_mada.get("ratio_efrs", 0)
        montant_ce_c = row_mada.get("montant_ce", 0)
        ...
        # Ressources
        # Titre Résultats scolaires
        res_title = Table([[Paragraph("EFFICIENCE ET ANALYSE DU GOULOT D'ETRANGLEMENT", centered_style)]], colWidths=[28*cm])
        res_title.setStyle(TableStyle([
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("ALIGN", (0, 0), (-1, -1), "CENTER"),
            ("GRID", (0, 0), (-1, -1), 1, colors.black),
        ...
        #--- Graphique de nuages
        y_cisco = self.y_zap_cisco()
        x_cisco = self.x_zap_cisco()

        # --- Données  ---
        x = [float(x["X"]) for x in x_cisco]  # ressources en %
        y = [float(y["Y"]) for y in y_cisco]  # ressources en %
        labels = [e["CISCO"] for e in x_cisco] #labels
        ...
        x_ecole, y_ecole = 0,0
        for c,z in zip(x_cisco, y_cisco) :
            if c["CODE_CISCO"] == CODE_CISCO:
                ax.text(c["X"]+1, z["Y"]+1, "*" + c["CISCO"], fontsize=10)
        ...
        plt.tight_layout()
        plt.savefig(f'{str(df.iloc[0]["CODE_CISCO"])}_sct.png')
        plt.close(fig)

        # --- Intégration dans le PDF ---
        img_scatter = RImage(f'{str(df.iloc[0]["CODE_CISCO"])}_sct.png', width=12*cm, height=8*cm)

        # Création de l’objet Image ReportLab
        # Goulot d'etranglement CICSO
        indicators = ['Taux de prommotion','Taux de retention', 'Pourcentage enseignant ayant diplome pédagogique','Ration élève par maître', 
                      'Elève par place assise','Ecole ayant éléctricité','Ecole ayant point d\'eau',
                       'Nombre d\' élève vivant à moins de 2km de leur école','Taux d\'admis au CEPE',

                    ]
        tpa = float(row_cisco.get("TPA",0))
        tret = float(row_cisco.get("txRetentionTotal",0))
        ratio_eq = float(row_cisco.get("ratio_eq",0))
        ratio_em = self.normaliser_valeur_0_100(val=float(row_cisco.get("ratio_em",0)), min_val=45, max_val=60)
        ratio_epa = self.normaliser_valeur_0_100(val=float(row_cisco.get("ratio_epa",0)), min_val=1, max_val=1)
        electricite = float(row_cisco.get("electricite",0))
        point_eau = float(row_cisco.get("point_eau",0))
        eleve_2km = float(row_cisco.get("eleve_2km",0))
        tx_admis = float(row_cisco.get("tx_admis",0))
        values = [tpa, tret, ratio_eq, ratio_em, ratio_epa, electricite, point_eau, eleve_2km, tx_admis]
        # Fonction pour déterminer la couleur en fonction de la valeur
        def get_color(value):
            if value < 50:
                return 'red'      # Rouge pour < 30%
            elif value <= 75:
                return 'gold'     # Jaune pour 30-60%
            else:
                return 'green'    # Vert pour > 60%

        #  liste de couleurs pour chaque barre
        bar_colors = [get_color(value) for value in values]
        fig, ax = plt.subplots(figsize=(10, 6))

        # ilay bar de progression par indicateur
        bars = ax.barh(indicators, values, color=bar_colors, edgecolor='black', alpha=0.8)

        #manoratra ny label valeur eo akaikin ny bar
        for i, (bar, value) in enumerate(zip(bars, values)):
            # Ajuster la couleur du texte selon la couleur de fond pour meilleure lisibilité
            text_color = get_color(value)
            ax.text(value + 1, bar.get_y() + bar.get_height()/2, f'{value}%', va='center', fontsize=8, fontweight='bold',color=text_color)

        #
        ax.set_xlabel('Pourcentage (%)', fontsize=11, fontweight='bold')
        ax.set_title('Indicateurs', fontsize=11, fontweight='bold', pad=20)
        ax.set_xlim(0, 120)  #  x-axis limit à 105% pour meilleure visibilité
        ...
        legend_elements = [
            Patch(facecolor='red', alpha=0.8, label='< 50% (Critique)'),
            Patch(facecolor='gold', alpha=0.8, label='50% - 75% (Attention)'),
            Patch(facecolor='green', alpha=0.8, label='> 75% (Bon)')
        ]
        ...
        # Export as PNG
        plt.savefig(f'{df.iloc[0]["CODE_CISCO"]}_goulot.png', dpi=300, bbox_inches='tight')
        img_bar = RImage(f'{df.iloc[0]["CODE_CISCO"]}_goulot.png', width=12*cm, height=8*cm)

        table_grgh = Table([[img_scatter,"",img_bar]], colWidths=[13*cm,0.5*cm,13*cm])
        ...
        # Diagnostique
        para = Paragraph("<para leftIndent=40><u><b>Diagnostic et Remarques sur les résultats de la CISCO :</b></u></para>", styles["Normal"])
        ...
        ae,az =row_cisco.get("txAbdGlobal",0),row_dren.get("txAbdGlobal",0)
        ...
        try:
            ...
            logger.error(f"Erreur lors de la génération du PDF CISCO {output_path} : {e}")
        try:
            inm = f'{df.iloc[0]["CODE_CISCO"]}_sct.png'
            ig = f'{df.iloc[0]["CODE_CISCO"]}_goulot.png'
            os.remove(inm)
            os.remove(ig)
            
        except Exception as e:
            logger.error(f"Erreur lors de la suppression du fichier {inm} : {e}")