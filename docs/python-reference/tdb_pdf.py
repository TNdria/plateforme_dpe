...
class PDF :
    ...
    def __init__(self, df_etab,df_etabs_in_zap, df_zap, df_cisco):
        self.df_etab = df_etab
        self.df_etabs_in_zap = df_etabs_in_zap
        self.df_zap = df_zap
    ...
    def y_etabs_zap(self):
        y = []
        for _,row in self.df_etabs_in_zap.iterrows() :
            data_y = {}
            data_y["NOM"] = row.get("NOM_ETAB","")
            data_y["CODE_ETAB"] = row.get("CODE_ETAB","")
            try:
                p = 100 - min(int(row.get("red_ensemble",0)),100)
                ret = float(row.get("txRetentionTotal",0))
                tpa = float(row.get("TPA",0))
                cepe = row.get("tx_admis",0) 
                yy = [p, ret,tpa, cepe]
                mean = round(stats.mean(yy), 1) 
                data_y["Y"] = mean
            except Exception as e:
                data_y["Y"] = 50
                print(f'erreur pour {data_y["CODE_ETAB"]}  => {e}')
    ...
    def x_etabs_zap(self):
        ...
        for _,row in self.df_etabs_in_zap.iterrows() :
            data_x = {}
            data_x["NOM"] = row.get("NOM_ETAB","")
            data_x["CODE_ETAB"] = row.get("CODE_ETAB","")
            try:
                rem = row.get("ratio_em",0) 
                rem = 0 if rem == 0  else self.normaliser_valeur_0_100(val=rem, min_val=45, max_val=60)
                req = float(row.get("ratio_eq",0) )
                eleve_2km = float(row.get("eleve_2km",0)) 
                eau = 100 if row.get("point_eau","NON")=="OUI" else 0
                elec = 100 if row.get("electricite","NON")=="OUI" else 0
                ce = float(row.get("ratio_ece",0))
                ...
                data_x["X"] = 50
                print(f'erreur pour x_etabs {data_x["CODE_ETAB"]} :=> {e}')
            x.append(data_x)
    ...
    def export_etab_pdf(self):
        df = self.df_etab.copy()
        # Nettoyer les quotes et apostrophes dans NOM_ETAB
        #"colors.lightgrey, colors.whitesmoke, colors.beige, colors.lightblue, colors.lavender"
        row_etab = self.df_etab.iloc[0]
        row_zap = self.df_zap.iloc[0]
        row_cisco = self.df_cisco.iloc[0]

        df["NOM_ETAB"] = df["NOM_ETAB"].str.replace('"', '', regex=False).str.replace("'", '', regex=False)
        df["ZAP"] = df["ZAP"].str.replace('"', '', regex=False).str.replace("'", '', regex=False).str.replace("/", '-', regex=False)
        
        pdf_name = df["NOM_ETAB"].iloc[0].replace("'", '')
        ...
        code_etab =  df.iloc[0]['CODE_ETAB']
        output_path = os.path.join(ZAP_DIR, f'{code_etab}_{pdf_name}.pdf')
        ...
        code = Paragraph(f'<b>CODE : {df.iloc[0]["CODE_ETAB"]}</b>', centered_style)
        ecole = Paragraph(f'<b>{df.iloc[0]["NOM_ETAB"]}</b>', centered_style)
        ...
        data_abandon = [
            ...
            entetes,
            ['', '', '', ''],
            ['CP1->CP2', f'{row_etab.get("txAbdcp1cp2","-1")}% ', f'{row_zap.get("txAbdcp1cp2","-1")}% ', f'{row_cisco.get("txAbdcp1cp2","-1")}% '],
            ['CP2->CE', f'{row_etab.get("txAbdcp2ce","-1")}% ', f'{row_zap.get("txAbdcp2ce","-1")}% ', f'{row_cisco.get("txAbdcp2ce","-1")}% '],
            ['CE->CM1', f'{row_etab.get("txAbdcecm1","-1")}% ', f'{row_zap.get("txAbdcecm1","-1")}% ', f'{row_cisco.get("txAbdcecm1","-1")}% '],
            ['CM1->CM2', f'{row_etab.get("txAbdcm1cm2","-1")}% ', f'{row_zap.get("txAbdcm1cm2","-1")}% ', f'{row_cisco.get("txAbdcm1cm2","-1")}% '],
            ['Ensemble', f'{row_etab.get("txAbdGlobal","-1")}% ', f'{row_zap.get("txAbdGlobal","-1")}% ', f'{row_cisco.get("txAbdGlobal","-1")}% '],
        ]
        #couleur pour txAbdon ensemble :
        e,z = float(str(row_etab.get("txAbdGlobal","-1")).replace(",",".")),float(str(row_zap.get("txAbdGlobal","-1")).replace(",","."))
        ...
        redcp1 = row_etab.get("red_CP1_g","-1") 
        redcp2 = row_etab.get("red_CP2_g","-1") 
        redce = row_etab.get("red_CE_g","-1")  
        redcm1 = row_etab.get("red_CM1_g","-1")  
        redcm2 = row_etab.get("red_CM2_g","-1") 
        
        data_pred = [
            [Paragraph('Pourcentage des redoublants', entete_style), '', '', ''],
            entetes,
            ['', '', '', ''],
            ['CP1', f'{redcp1}%', row_zap.get('red_CP1_g','-1'), row_cisco.get('red_CP1_g','-1')],
            ['CP2', f'{redcp2}%', row_zap.get('red_CP2_g','-1'), row_cisco.get('red_CP2_g','-1')],
            ['CE', f'{redce}%', row_zap.get('red_CE_g','-1'), row_cisco.get('red_CE_g','-1')],
            ['CM1', f'{redcm1}%', row_zap.get('red_CM1_g','-1'), row_cisco.get('red_CM1_g','-1')],
            ['CM2', f'{redcm2}%', row_zap.get('red_CM2_g','-1'), row_cisco.get('red_CM2_g','-1')],
        ]
        ...
        rg_e = float(str(row_etab.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_e = float(str(row_etab.get("txRetentionFilles","0")).strip().replace(",","."))
        r_e = float(str(row_etab.get("txRetentionTotal","0")).strip().replace(",","."))
        rg_z = float(str(row_zap.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_z = float(str(row_zap.get("txRetentionFilles","0")).strip().replace(",","."))
        r_z = float(str(row_zap.get("txRetentionTotal","0")).strip().replace(",","."))
        rg_c = float(str(row_cisco.get("txRetentionGarcons","0")).strip().replace(",","."))
        rf_c = float(str(row_cisco.get("txRetentionFilles","0")).strip().replace(",","."))
        r_c = float(str(row_cisco.get("txRetentionTotal","0")).strip().replace(",","."))
        ...
        ratio_e = safe_ratio(rf_e, rg_e)
        ratio_z = safe_ratio(rf_z, rg_z)
        ratio_c = safe_ratio(rf_c, rg_c)
        dp_e = ("-" if ratio_e is np.nan or not np.isfinite(ratio_e) or 0.97 <= ratio_e <= 1.03 else (img_f if ratio_e < 0.97 else img_g))
        ...
        rg_e = row_etab.get("red_garcons",-1) 
        rf_e = row_etab.get("red_fille",-1) 
        r_e = row_etab.get("red_ensemble",-1) 

        rg_z = row_zap.get("red_garcons",-1) 
        rf_z = row_zap.get("red_fille",-1) 
        r_z = row_zap.get("red_ensemble",-1) 


        rg_c = row_cisco.get("red_garcons",-1) 
        rf_c = row_cisco.get("red_fille",-1) 
        r_c = row_cisco.get("red_ensemble",-1) 
        ...
        ratio_e2 = safe_ratio(rf_e, rg_e)
        ratio_z2 = safe_ratio(rf_z, rg_z)
        ratio_c2 = safe_ratio(rf_c, rg_c)
        dp_e = ("-" if ratio_e2 is np.nan or not np.isfinite(ratio_e2) or 0.97 <= ratio_e2 <= 1.03 else (img_g if ratio_e2 < 0.97 else img_f))
        ...
        entetes = [
            ["Matières", "École", "", "ZAP", "", "CISCO", ""],
            ["", "SM", "Note >= 10", "SM", "Note >= 10", "SM", "Note >= 10"]
        ]
        tx_admis_g =  row_etab.get("tx_admis_g", 0)
        tx_admis_f = row_etab.get("tx_admis_f", 0)
        tx_admis = row_etab.get("tx_admis", 0)

        tx_admis_g_z =  row_zap.get("tx_admis_g", 0)
        tx_admis_f_z = row_zap.get("tx_admis_f", 0)
        tx_admis_z = row_zap.get("tx_admis", 0)

        tx_admis_g_c =  row_cisco.get("tx_admis_g", 0)
        tx_admis_f_c = row_cisco.get("tx_admis_f", 0)
        tx_admis_c = row_cisco.get("tx_admis", 0)

        ratio_ee = safe_ratio(tx_admis_f, tx_admis_g)
        ratio_ez = safe_ratio(tx_admis_g_z, tx_admis_f_z)
        ratio_ec = safe_ratio(tx_admis_g_c, tx_admis_f_c)
        dp_e = ("-" if ratio_ee is np.nan or not np.isfinite(ratio_ee) or 0.97 <= ratio_ee <= 1.03 else (img_f if ratio_ee < 0.97 else img_g))
        dp_z = ("-" if ratio_ez is np.nan or not np.isfinite(ratio_ez) or 0.97 <= ratio_ez <= 1.03 else (img_f if ratio_ez < 0.97 else img_g))
        dp_c = ("-" if ratio_ec is np.nan or not np.isfinite(ratio_ec) or 0.97 <= ratio_ec <= 1.03 else (img_f if ratio_ec < 0.97 else img_g))
        
        data_exam = [
            [titre_ex, "", "", "", "", "", ""], #0 
            ["Matières", "École", "", "ZAP", "", "CISCO", ""], #1
            ["", "SM", " >= 10", "SM", " >= 10", "SM", " >= 10"], #2
            ["Malagasy", row_etab.get("sm_mlg",0), row_etab.get("sup_10_mlg",0), row_zap.get("sm_mlg",0), row_zap.get("sup_10_mlg",0),row_cisco.get("sm_mlg",0), row_cisco.get("sup_10_mlg",0)], #3
            ["  -Fahazoana lahatsoratra", "", "", "", "", "", ""], #4
            ["  -Fitsipika", "", "", "", "", "", ""], #5
            ["  -Fanazarana hanoratra", "", "", "", "", "", ""], #6
            ["Français", row_etab.get("sm_fr",0), row_etab.get("sup_10_fr",0), row_zap.get("sm_fr",0), row_zap.get("sup_10_fr",0),row_cisco.get("sm_fr",0), row_cisco.get("sup_10_fr",0)], #7
            ["Mathématiques", row_etab.get("sm_maths",0), row_etab.get("sup_10_maths",0), row_zap.get("sm_maths",0), row_zap.get("sup_10_maths",0),row_cisco.get("sm_maths",0), row_cisco.get("sup_10_maths",0)], #8
            ["  -Opération", row_etab.get("sm_op",0), row_etab.get("sup_10_op",0), row_zap.get("sm_op",0), row_zap.get("sup_10_op",0),row_cisco.get("sm_op",0), row_cisco.get("sup_10_op",0)], #9
            ["  -Problème", row_etab.get("sm_probleme",0), row_etab.get("sup_10_probleme",0), row_zap.get("sm_probleme",0), row_zap.get("sup_10_probleme",0),row_cisco.get("sm_probleme",0), row_cisco.get("sup_10_probleme",0)], #10
            ["Histroire", row_etab.get("sm_tfm",0), row_etab.get("sup_10_tfm",0), row_zap.get("sm_tfm",0), row_zap.get("sup_10_tfm",0),row_cisco.get("sm_tfm",0), row_cisco.get("sup_10_tfm",0)], #11
            ["Geographie", row_etab.get("sm_geo",0), row_etab.get("sup_10_geo",0), row_zap.get("sm_geo",0), row_zap.get("sup_10_geo",0),row_cisco.get("sm_geo",0), row_cisco.get("sup_10_geo",0)], #12
            ["SVT", row_etab.get("sm_svt",0), row_etab.get("sup_10_svt",0), row_zap.get("sm_svt",0), row_zap.get("sup_10_svt",0),row_cisco.get("sm_svt",0), row_cisco.get("sup_10_svt",0)], #13
            [ttl2, "", "", "", "", "", ""], #14
            ...
            ["Disparité au dépens des", dp_e, "", dp_z, "", dp_c, ""], #18
        ]
        table_exam = Table(data_exam, colWidths=[5*cm, 1.41*cm, 1.41*cm, 1.41*cm, 1.41*cm, 1.41*cm, 1.43*cm])
        bc_mlg = colors.red if row_etab.get("sm_mlg",0) < 10 else colors.white
        bc_fr = colors.red if row_etab.get("sm_fr",0) < 10 else colors.white
        bc_maths = colors.red if row_etab.get("sm_maths",0) < 10 else colors.white
        bc_op = colors.red if row_etab.get("sm_op",0) < 10 else colors.white
        bc_pb = colors.red if row_etab.get("sm_probleme",0) < 10 else colors.white
        bc_tfm = colors.red if row_etab.get("sm_tfm",0) < 10 else colors.white
        bc_geo = colors.red if row_etab.get("sm_geo",0) < 10 else colors.white
        bc_svt = colors.red if row_etab.get("sm_svt",0) < 10 else colors.white
        ...
        nbr_eleves = row_etab.get("nombre_eleves", 0)
        ens_classe = row_etab.get("ens_classe", 0)
        ens_im = row_etab.get("ens_im", 0)
        fram_sub = row_etab.get("fram_sub", 0)
        fram_nonsub = row_etab.get("fram_nonsub", 0)
        nombre_section = row_etab.get("nombre_section", 0)
        ecole_continue = row_etab.get("ecole_continue", 0)
        eleve_2km = row_etab.get("eleve_2km", 0)
        point_eau = row_etab.get("point_eau", 0)
        electricite = row_etab.get("electricite", 0)

        nbr_eleves_z = row_zap.get("nombre_eleves", 10)
        ens_classe_z = row_zap.get("ens_classe", 10)
        ens_im_z = row_zap.get("ens_im", 10)
        fram_sub_z = row_zap.get("fram_sub", 10)
        fram_nonsub_z = row_zap.get("fram_nonsub",10)
        nombre_section_z = row_zap.get("nombre_section", 10)
        ecole_continue_z = row_zap.get("ecole_continue", 10)
        eleve_2km_z = row_zap.get("eleve_2km", 0)
        point_eau_z = row_zap.get("point_eau", 0)
        electricite_z = row_zap.get("electricite", 0)

        nbr_eleves_c = row_cisco.get("nombre_eleves", 10)
        ens_classe_c = row_cisco.get("ens_classe", 10)
        ens_im_c = row_cisco.get("ens_im", 10)
        fram_sub_c = row_cisco.get("fram_sub", 10)
        fram_nonsub_c = row_cisco.get("fram_nonsub",10)
        nombre_section_c = row_cisco.get("nombre_section", 10)
        ecole_continue_c = row_cisco.get("ecole_continue", 10)
        eleve_2km_c = row_cisco.get("eleve_2km", 0)
        point_eau_c = row_cisco.get("point_eau", 0)
        electricite_c = row_cisco.get("electricite", 0)
        ...
        data_g = [
            ...
            ["Contexte","","",""],
            ["Écoles continues",f"{ecole_continue}",f"{ecole_continue_z}%",f"{ecole_continue_c}%"],
            ["Elèves vivant à moins de 2km de leur école",f"{eleve_2km}%",f"{eleve_2km_z}",f"{eleve_2km_c}%"],
        ...
        ratio_em = row_etab.get("ratio_em", 0)
        ratio_eq = row_etab.get("ratio_eq", 0)
        ratio_cpsdc = row_etab.get("ratio_cpsdc", 0)
        ratio_epa = row_etab.get("ratio_epa", 0)
        ratio_wc_com = row_etab.get("ratio_wc_com", 0)
        ratio_wc_f = row_etab.get("ratio_wc_f", 0)
        ratio_emlg = row_etab.get("ratio_emlg", 0)
        ratio_etmths = row_etab.get("ratio_etmths", 0)
        ratio_efrs = row_etab.get("ratio_efrs", 0)
        montant_ce = row_etab.get("montant_ce", 0)

        ratio_em_z = row_zap.get("ratio_em", 0)
        ratio_eq_z = row_zap.get("ratio_eq", 0)
        ratio_cpsdc_z = row_zap.get("ratio_cpsdc", 0)
        ratio_epa_z = row_zap.get("ratio_epa", 0)
        ratio_wc_com_z = row_zap.get("ratio_wc_com", 0)
        ratio_wc_f_z = row_zap.get("ratio_wc_f", 0)
        ratio_emlg_z = row_zap.get("ratio_emlg", 0)
        ratio_etmths_z = row_zap.get("ratio_etmths", 0)
        ratio_efrs_z = row_zap.get("ratio_efrs", 0)
        montant_ce_z = row_zap.get("montant_ce", 0)

        ratio_em_c = row_cisco.get("ratio_em", 0)
        ratio_eq_c = row_cisco.get("ratio_eq", 0)
        ratio_cpsdc_c = row_cisco.get("ratio_cpsdc", 0)
        ratio_epa_c = row_cisco.get("ratio_epa", 0)
        ratio_wc_com_c = row_cisco.get("ratio_wc_com", 0)
        ratio_wc_f_c = row_cisco.get("ratio_wc_f", 0)
        ratio_emlg_c = row_cisco.get("ratio_emlg", 0)
        ratio_etmths_c = row_cisco.get("ratio_etmths", 0)
        ratio_efrs_c = row_cisco.get("ratio_efrs", 0)
        montant_ce_c = row_cisco.get("montant_ce", 0)
        ...
        data_d = [
            ...
            ['Nombred\'lèves par manuel Maths',f'{ratio_etmths}',f'{ratio_etmths_z}',f'{ratio_etmths_c}'], #8
            ['Nombred\'lèves par manuel Francçais',f'{ratio_efrs}',f'{ratio_efrs_z}',f'{ratio_efrs_c}'], #9
            ['Ressources financières en Ariary','','',''], #10
            ['Caisse écoles/Subvention/Autres',f'{montant_ce}',f'{montant_ce_z}',f'{montant_ce_c}'], #11
        ]
        ...
        plt.tight_layout()
        plt.savefig(f'{str(df.iloc[0]["CODE_ETAB"])}_sct.png')
        plt.close(fig)

        # --- Intégration dans le PDF ---
        img_scatter = RImage(f'{str(df.iloc[0]["CODE_ETAB"])}_sct.png', width=20*cm, height=8*cm)

        # Création de l’objet Image ReportLab
        #img_line = RImage(f'{str(df.iloc[0]["CODE_ETAB"])}_line.png', width=12*cm, height=8*cm)
        ...
        # Diagnostique
        para = Paragraph("<para leftIndent=40><u><b>Diagnostic et Remarques sur les résultats de l'école:</b></u></para>", styles["Normal"])
        ...
        ae,az =row_etab.get("txAbdGlobal",0),row_zap.get("txAbdGlobal",0)
        ...
        try:
            ...
            os.remove(f'{str(df.iloc[0]["CODE_ETAB"])}_sct.png')
        except Exception as e:
