...
import { MapPin, School, Users, Building2, AlertTriangle, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
const ORSCollege = () => {
  ...
  const filteredPrimaires = useMemo(
    () => primaires.filter(p => (p.SECTEUR === 0 ? layerVisibility.publiques : layerVisibility.prives)),
  ...
  const stats = useMemo(() => {
    ...
    const totalPrimaires = primaires.length;
    const eligiblesReconstruction = colleges.filter(c => c.eligible_reconstruction).length;
    const eligiblesRehab = colleges.filter(c => c.eligible_rehabilitation).length;
    return { totalColleges, totalPrimaires, eligiblesReconstruction, eligiblesRehab };
  }, [colleges, primaires]);
  ...
  const handleDownload = useCallback(() => {
    ...
    const ciscoLabel = selectedCisco > 0
      ...
      : 'tous';
    const filename = `ORS_COLLEGE_${drenLabel}_${ciscoLabel}_${new Date().toISOString().slice(0,10)}.csv`;
    const ok = downloadAsCsv(all, filename, ORS_CSV_COLUMNS);
    if (ok) toast.success(`${all.length} établissements exportés`);
  }, [colleges, primaires, drens, ciscos, selectedDren, selectedCisco]);
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Breadcrumb */}
      <div className="px-4 py-2 bg-gradient-to-r from-green-500/10 to-transparent border-b border-border">
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-muted-foreground flex items-center gap-2">
            <Building2 className="w-4 h-4" />
            CARTE SCOLAIRE / ORS / ORS COLLEGE
          </span>
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden">
        ...
        <div className="w-80 flex-shrink-0 space-y-4 overflow-y-auto">
          <MapFilters
            ...
            onDownload={colleges.length > 0 ? handleDownload : undefined}
            loading={loading}
            searchItems={[...colleges, ...primaires]}
          ...
          {/* Category Filter */}
          {(colleges.length > 0 || primaires.length > 0) && (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-muted/50 rounded-lg p-3">
                  ...
                  <div className="font-semibold">{selectedEtablissement.effectifs || selectedEtablissement.eff_2024 || '-'}</div>
                </div>
              </div>
