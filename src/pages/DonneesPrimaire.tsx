...
const DonneesPrimaire = () => {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const data = await donneesApi.getEtabN1(dren, cisco, commune, zap, secteur);
  ...
  const handleExport = () => {
    ...
    const headers = ['Code', 'Établissement', 'DREN', 'CISCO', 'ZAP', 'Commune', 'Fokontany', 'Secteur', 'T1', 'T2', 'T3', 'T4', 'T5', 'Total'];
    const rows = etablissements.map(e => [
      ...
      e.eff_t4 || 0,
      e.eff_t5 || 0,
      (e.eff_t1 || 0) + (e.eff_t2 || 0) + (e.eff_t3 || 0) + (e.eff_t4 || 0) + (e.eff_t5 || 0)
    ]);
    ...
    a.href = url;
    a.download = 'donnees_primaire.csv';
    a.click();
    toast.success('Export CSV téléchargé');
  };

  const totalEleves = etablissements.reduce((acc, e) => 
    acc + (e.eff_t1 || 0) + (e.eff_t2 || 0) + (e.eff_t3 || 0) + (e.eff_t4 || 0) + (e.eff_t5 || 0), 0);
        ...
        <Card className="h-full flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between py-3 px-3 sm:px-6">
            <CardTitle className="flex items-center gap-2 text-sm sm:text-lg">
              ...
              Écoles primaires ({etablissements.length})
            </CardTitle>
                  ...
                  <Table>
                    <TableHeader className="sticky top-0 bg-card z-10">
                      <TableRow>
                        ...
                        <TableHead className="text-right">T5</TableHead>
                    ...
                    <TableBody>
                      {pagination.paginatedData.map((etab, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-right">{etab.eff_t1 || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_t2 || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_t3 || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_t4 || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_t5 || 0}</TableCell>
                          <TableCell className="text-right font-semibold">
                            {(etab.eff_t1 || 0) + (etab.eff_t2 || 0) + (etab.eff_t3 || 0) + (etab.eff_t4 || 0) + (etab.eff_t5 || 0)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
...
export default DonneesPrimaire;
