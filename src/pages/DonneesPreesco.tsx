...
const DonneesPreesco = () => {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const data = await donneesApi.getEtabN0(dren, cisco, commune, zap, secteur);
  ...
  const handleExport = () => {
    ...
    const headers = ['Code', 'Établissement', 'DREN', 'CISCO', 'ZAP', 'Commune', 'Fokontany', 'Secteur', 'PS', 'MS', 'GS', 'Total'];
    const rows = etablissements.map(e => [
      ...
      e.eff_ms || 0,
      e.eff_gs || 0,
      (e.eff_ps || 0) + (e.eff_ms || 0) + (e.eff_gs || 0)
    ]);
    ...
    a.href = url;
    a.download = 'donnees_prescolaire.csv';
    a.click();
    toast.success('Export CSV téléchargé');
  };

  const totalEleves = etablissements.reduce((acc, e) => acc + (e.eff_ps || 0) + (e.eff_ms || 0) + (e.eff_gs || 0), 0);
    ...
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 bg-muted/50 border-b border-border">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <DataActionsBar table="fpe_a1" tableLabel="Établissements (Préscolaire)" compact />
        </div>
      </div>
        ...
        <Card className="h-full flex flex-col">
          <CardHeader className="flex flex-row items-center justify-between py-3 px-3 sm:px-6">
            <CardTitle className="flex items-center gap-2 text-sm sm:text-lg">
              ...
              Préscolaires ({etablissements.length})
            </CardTitle>
                    ...
                    <TableBody>
                      {pagination.paginatedData.map((etab, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-right">{etab.eff_ps || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_ms || 0}</TableCell>
                          <TableCell className="text-right">{etab.eff_gs || 0}</TableCell>
                          <TableCell className="text-right font-semibold">
                            {(etab.eff_ps || 0) + (etab.eff_ms || 0) + (etab.eff_gs || 0)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
