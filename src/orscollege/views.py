...
def index(request):
    drens = get_dren()
    return render(request,template_name="orscollege.html", context={"dren":drens})
...
def get_layer_etabN2(request,code_dren,code_cisco):
    q = """ SELECT * FROM v_layer_besoins_n2 {0} {1} """
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(f' WHERE "SECTEUR" = 0 AND "CODE_DREN" = ', code_dren)
    else:
        q = q.format(f' WHERE  "SECTEUR" = 0 AND "CODE_CISCO" = ', code_cisco)
...
#recuperer layer des etablissement n2S0
def get_layers_besoins_n2(request,code_dren,code_cisco):
    q = """ SELECT * FROM v_layer_besoins_n2 {0} {1} """
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(' WHERE "CODE_DREN" = ', code_dren)
    else:
...
#recuperer layer des etablissement n2S0
def download_besoins_n2(request,code_dren,code_cisco):
    q = """ SELECT  
                vb."NOM_ETAB",
                vb."CODE_ETAB",
                vb."ANNEE_SCOLAIRE",
             ...
             FROM v_layer_besoins_n2 vb {0} {1} """
    cd = ""
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(' WHERE "CODE_DREN" = ', code_dren)
    ...
    res = queryset_to_csv(q, f"donnees_ors_college_{cd}")
    return res

#obtenir la couche nouvelle creation depuis le la view materialize v_layer_ncn2
def get_layer_ncn2(request, code_dren,code_cisco):
    q = """ SELECT * FROM v_layer_ncn2 {0} {1} """
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(' WHERE "CODE_DREN" = ', code_dren)
    else:
        q = q.format(' WHERE "CODE_CISCO" = ', code_cisco)
