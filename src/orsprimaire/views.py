...
def index(request):
    drens = get_dren()
    return render(request,template_name="orsprimaire.html", context={"dren":drens})
...
"""
def get_all_village(request,code_dren,code_cisco):
    q = """ SELECT * FROM sig_village {0} {1} ORDER BY name """
    if (code_dren > 0 and code_cisco == 0):
...
#recuperer layer des etablissement N1S0
def get_layers_besoins_n1(request,code_dren,code_cisco):
    q = """ SELECT * FROM v_layer_besoins_n1 {0} {1} """
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(' WHERE "CODE_DREN" = ', code_dren)
    else:
...
#recuperer layer des etablissement N1S0
def download_besoins_n1(request,code_dren,code_cisco):
    q = """ SELECT  
                vb."NOM_ETAB",
                vb."CODE_ETAB",
                vb."ANNEE_SCOLAIRE",
                ...
                vb."CODE_CISCO",
                vb."ZAP",
                vb."CATEGORIE_COMMUNE",
                CAST(vb.effectifs AS INTEGER),
                CAST(vb.gp_sections AS INTEGER),
                ...
                CASE WHEN (vb.sdc_requis > (vb.sdc_be + vb.sdc_me))  THEN CAST((vb.sdc_requis - (vb.sdc_be + vb.sdc_me))  AS INTEGER) ELSE 0 END AS BESOINS_EXTENSION,
             ...
             FROM v_layer_besoins_n1 vb 
            INNER JOIN v_dren vd ON vd."CODE_DREN" = vb."CODE_DREN"
             INNER JOIN v_cisco vc ON vc."CODE_CISCO" = vb."CODE_CISCO"
             {0} {1} """
    cd = ""
    if (code_dren > 0 and code_cisco == 0):
        q = q.format(' WHERE vb."CODE_DREN" = ', code_dren)
        cd = "dren_" + str(code_dren)
    else:
        q = q.format(' WHERE vb."CODE_CISCO" = ', code_cisco)
        cd = "cisco_" + str(code_cisco)

    res = queryset_to_csv(q, f"donnees_ors_pimaire_{cd}.csv")
    return res



#recuperer layer des etablissement N1S0
def download_besoins_nationale_n1(request):
    q = """ SELECT  
                vb."ANNEE_SCOLAIRE",
                vb."NOM_ETAB",
                vb."CODE_ETAB",
                ...
                vb."ZAP",
                vb."CATEGORIE_COMMUNE",
                CAST(vb.effectifs AS INTEGER) AS EFFECTIFS,
                CAST(vb.gp_sections AS INTEGER) AS GP,
                CAST(vb.sdc_be AS INTEGER) AS SDC_BON_ETAT,
                CAST(vb.sdc_me AS INTEGER) AS SDC_MAUVAIS_ETAT,
                CAST(vb.sdc_be + vb.sdc_me AS INTEGER) AS TOTAL_SDC,
                CASE WHEN (vb.eligible_reconstruction > 0)  THEN 'OUI' ELSE 'NON' END AS ELIGIBLE_RECONSTRUCTION,
                /*CASE WHEN (vb.eligible_rehabilitation > 0)  THEN 'OUI' ELSE 'NON' END AS ELIGIBLE_REHABILITATION,*/
                CAST(vb.sdc_requis AS INTEGER) AS SDC_REQUIS,
                CASE WHEN (vb.sdc_requis > (vb.sdc_be + vb.sdc_me))  THEN CAST((vb.sdc_requis - (vb.sdc_be + vb.sdc_me))  AS INTEGER) ELSE 0 END AS BESOINS_EXTENSION,
                CAST(vb.places AS INTEGER) PLACES_DISPONIBLES,
                CASE WHEN (vb.effectifs < vb.places) THEN 0 ELSE ROUND((vb.effectifs - vb.places) / 2, 0) END AS BESOIN_TABLE_2_PLACES
             FROM v_layer_besoins_n1 vb  
    ...
    res = queryset_to_csv(q, f"donnees_ors_primaire_nationale.csv")
    return res


#obtenir la couche nouvelle creation depuis le la view materialize v_layer_ncn1
def get_nc(request):
    q = """ SELECT * FROM v_layer_ncn1 """
    res = to_dicts_json(q)
    return res

def get_layer_villages_exclus(request,code_dren,code_cisco):
    q = """ WITH cte AS (
